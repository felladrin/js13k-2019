...
export class GameStorage {
  ...
  public save(data: GameData): void {
    ...
    const gameDataAsString = JSON.stringify(data),
      cipher = this.getCipher(gameName),
      cipheredGameData = cipher(gameDataAsString);
  ...
  public load(): GameData {
    ...
    const decipher = this.getDecipher(gameName),
      gameDataAsString = decipher(cipheredGameData);
  ...
  private getCipher(salt: string): (text: string) => string {
    const textToChars: (text) => number[] = (text) => text.split("").map((c) => c.charCodeAt(0)),
      byteHex: (n) => string = (n) => `0${Number(n).toString(16)}`.substr(-2),
      applySaltToChar: (code) => number = (code) => textToChars(salt).reduce((a, b) => a ^ b, code);
